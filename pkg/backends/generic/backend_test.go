	...
	tests := []struct {
	...
	}{
		{
			...
			want: loadTestV1.LoadTestSpec{
				...
				TargetURL:       "http://my-app.my-domain.com",
				WorkerConfig:    loadTestV1.ImageDetails("someimage:latest"),
			},
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			spec := &loadTestV1.LoadTestSpec{
				...
				WorkerConfig:    loadTestV1.ImageDetails(tt.args.workerImage),
			}
			...
			assert.Equal(t, tt.want.WorkerConfig, spec.WorkerConfig)
